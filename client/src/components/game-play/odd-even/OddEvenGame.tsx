...
import type { OddEvenGameState, OddEvenGuess } from '@shared/games/odd-even/schema';
...
interface OddEvenGameProps {
  gameState: OddEvenGameState;
  room: RoomWithPlayers;
  currentUser: User | null;
  isParticipant: boolean;
  moveSubmitted: boolean;
  selectedChoice: OddEvenGuess | null;
  onChoiceSelect: (choice: OddEvenGuess) => void;
  gamePlayers: Array<{
    userId: string;
    user: {
      id: string;
    ...
    <>
      ...
      {gameState.gameStatus === 'waiting_for_moves' && isParticipant && (
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>
              {ODD_EVEN_CONFIG.description}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              {ODD_EVEN_CONFIG.choices!.map((choice) => {
                const displayText = choice === 'odd' ? '홀수' : '짝수';
                ...
                return (
                  ...
                  >
                    <div className="text-center">
                      <div className="text-4xl font-bold">{displayText}</div>
                    </div>
                  </Button>
                );
              })}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm text-muted-foreground mb-2">
                        정답: {round.targetAnswer === 'odd' ? '홀수' : '짝수'}
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {gamePlayers.map(player => {
                          const playerChoice = round.playerChoices[player.userId];
                          const isCorrect = playerChoice === round.targetAnswer;
                          const isCurrentPlayer = player.userId === currentUser?.id;
                          
                          const displayChoice = playerChoice === 'odd' ? '홀수' : '짝수';
